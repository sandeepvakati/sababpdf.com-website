const fs = require('fs');
const lines = fs.readFileSync('lib/pdfUtils.js', 'utf8').split('\n');

const start = lines.findIndex(l => l.includes('export async function addWatermark'));
const end = lines.findIndex((l, i) => i > start && l.trim() === '}' && lines[i-1].includes('return new Blob'));

if (start !== -1 && end !== -1) {
  lines.splice(start, end - start + 1, \export async function addWatermark(file, options = {}) {
  const {
    text = 'SababPDF.com',
    opacity = 0.5,
    color = '#000000',
  ...
  fs.writeFileSync('lib/pdfUtils.js', lines.join('\\n'));
  console.log('Success splice');
} else {

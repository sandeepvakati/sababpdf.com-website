...
export default function PdfToExcelPage() {
  ...
  const process = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/convert/pdf-to-excel`, { method: 'POST', body: formData });
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error || `Error ${res.status}`);
      setResult(await res.blob());
      toast.success('Converted to Excel!');
  ...
  return (
    <ToolLayout tool={tool}>
      {!result ? (
        !file ? (
          <FileUploader onFiles={f => setFile(f[0])} accept=".pdf" label="Drop PDF to extract data" description="Tables will be extracted to Excel" />
        ) : (
          <>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={process} disabled={processing} className="btn-primary" style={{ fontSize: 16, padding: '12px 32px' }}>
                {processing ? '⏳ Extracting...' : '📊 Convert to Excel'}
              </button>
        ...
        <div style={{ textAlign: 'center', padding: '48px' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-syne)', fontWeight: 800, fontSize: 28, marginBottom: 24 }}>Converted to Excel!</h2>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
            <button onClick={() => downloadBlob(result, file.name.replace('.pdf', '.xlsx'))} className="btn-primary" style={{ fontSize: 16, padding: '14px 36px' }}>⬇️ Download XLSX</button>

...
const ScanToPdf = () => {
    ...
    useEffect(() => {
        const initSession = async () => {
            ...
            // Need to make sure window is defined for origin
            setMobileUrl(`${window.location.origin}/scan-mobile/${newSessionId}`);
            ...
            <div className="max-w-5xl mx-auto px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                        ...
                        {initError ? (
                            ...
                            <>
                                ...
                                <div className="bg-white p-4 rounded-xl shadow-inner border border-gray-200 mb-6">
                                    {mobileUrl && <QRCodeSVG value={mobileUrl} size={200} level={"H"} />}
                                </div>
                    ...
                    <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 h-full flex flex-col">
                        ...
                        {scannedImages.length === 0 ? (
                            <div className="flex-grow flex flex-col items-center justify-center text-gray-300 min-h-[300px] border-2 border-dashed border-gray-100 rounded-xl bg-gray-50/50">
                                <div className="w-20 h-20 bg-white shadow-sm rounded-full flex items-center justify-center mb-4 border border-gray-100">
                                    <Smartphone className="w-10 h-10 text-gray-400" />
                                </div>
                                <p className="text-sm font-medium">Photos taken on your phone will appear here instantly</p>
                            </div>
                        ) : (
                            <div className="flex-grow">
                                <div className="grid grid-cols-2 gap-4 mb-6 max-h-[400px] overflow-y-auto p-2">
                                    {scannedImages.map((img, idx) => (
                                        <div key={idx} className="relative group rounded-lg overflow-hidden border border-gray-200 aspect-[3/4] shadow-sm">
                                            <img src={img.url} alt="Scan" className="w-full h-full object-cover" />
                                            <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-sm text-white text-xs py-1.5 font-medium text-center">
                                                Page {idx + 1}
                                            </div>
                                        </div>
                                    ))}
                                ...
                                <button
                                    ...
                                    className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-4 rounded-xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 border border-orange-600/20"
                                >
                                    {isProcessing ? (
                                        <>

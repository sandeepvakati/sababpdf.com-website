...
export default function PptToPdfPage() {
  ...
  return (
    <ToolLayout tool={tool}>
      {!result ? (
        !file ? (
          <FileUploader onFiles={f => setFile(f[0])} accept=".ppt,.pptx" label="Drop PowerPoint here" description=".PPT and .PPTX files supported" />
        ) : (
          <>
        ...
        <div style={{ textAlign: 'center', padding: '48px' }}>
          ...
          <h2 style={{ fontFamily: 'var(--font-syne)', fontWeight: 800, fontSize: 28, marginBottom: 24 }}>Done!</h2>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
            <button onClick={() => downloadBlob(result, file.name.replace(/\.pptx?$/i, '.pdf'))} className="btn-primary" style={{ fontSize: 16, padding: '14px 36px' }}>⬇️ Download PDF</button>

...
const FeatureCard = ({ title, description, icon: Icon, href, color }) => {
    return (
        <Link href={href} className="group bg-white p-4 md:p-6 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 border border-gray-100 hover:border-black block h-full">
            <div className="flex flex-row md:flex-col h-full items-center md:items-start gap-4 md:gap-3">
                ...
                <div className="flex-grow">
                    ...
                    <p className="text-gray-500 leading-relaxed text-xs md:text-base line-clamp-2 md:line-clamp-3">{description}</p>
                </div>
            </div>
        </Link>
    );

...
const target = `export async function addWatermark(file, { text = 'WATERMARK', opacity = 0.3, color = '#808080', fontSize = 60, rotation = -45 }) {
...
if (content.includes(target)) {
  fs.writeFileSync('lib/pdfUtils.js', content.replace(target, replacement));
  console.log('Success exact match');
} else {
  console.log('Exact match failed. Check target string.');
}

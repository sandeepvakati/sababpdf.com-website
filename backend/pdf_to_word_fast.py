#!/usr/bin/env python3
"""
Fast PDF to Word Converter - Optimized for Speed
Fixed the performance bottleneck in table detection

KEY FIX: Open pdfplumber document ONCE and reuse across all pages
         instead of reopening for every single page
"""
...
class FastPDFToWordConverter:
    """
    Fast PDF to Word converter with optimized table detection.
    KEY OPTIMIZATION: Open pdfplumber document once, reuse for all pages.
    """
    ...
    def __init__(self, pdf_path: str, output_path: str, mode: str = 'layout'):
        ...
        self.pdf_doc = None
        self.page_count = 0
        self.pdfplumber_doc = None  # Cache pdfplumber document
    ...
    def open_pdfplumber(self) -> bool:
        """Open PDF document with pdfplumber ONCE for all pages"""
        if not PDFPLUMBER_AVAILABLE:
            return False
        
        try:
            self.pdfplumber_doc = pdfplumber.open(self.pdf_path)
            logger.info("Opened pdfplumber document (once for all pages)")
            return True
        except Exception as e:
    ...
    def extract_images(self, page_num: int) -> List[Dict]:
        ...
        try:
            ...
            # Also extract images from text dict
    ...
    def detect_table_regions(self, page_num: int) -> List[Dict]:
        ...
        KEY OPTIMIZATION: Uses self.pdfplumber_doc which is opened ONCE
        instead of reopening the PDF file for every page.
        """
        table_regions = []
        ...
        try:
            ...
            # Strategy 1: Use CACHED pdfplumber document (opened once)
            if PDFPLUMBER_AVAILABLE and self.pdfplumber_doc:
                try:
                    if page_num < len(self.pdfplumber_doc.pages):
                        ...
                        logger.info(f"Page {page_num + 1}: Found {len(tables)} tables via pdfplumber (cached)")
                except Exception as e:
                    logger.warning(f"pdfplumber table detection failed on page {page_num}: {e}")

            # Strategy 2: Detect grid-like text patterns using PyMuPDF (fallback)
            text_dict = page.get_text("dict")
            ...
            if len(text_lines) >= 3:
                ...
                if len(rows) >= 2:
                    ...
                    for row in rows[:5]:
                        for item in row:
                            col_positions.append(item['bbox'][0])
    ...
    def create_layout_document(self) -> bool:
        """
        Create a Word document with layout preservation.
        HYBRID APPROACH:
        - For pages with tables/formulas: render as image
        - For text-only pages: extract as editable text
        """
    ...
    def _add_page_as_image(self, doc, page_num: int, section) -> None:
        ...
        try:
            ...
            if img:
                ...
                # Get usable dimensions (section.page_width is already in Inches)
    ...
    def _add_page_as_editable_text(self, doc, page_num: int, text_blocks: List[Dict]) -> None:
        """Add extracted text blocks as editable text"""
        try:
    ...
    def convert(self) -> Tuple[bool, str]:
        ...
        # KEY OPTIMIZATION: Open pdfplumber ONCE here
...
def convert_pdf_to_word(pdf_path: str, output_path: str, mode: str = 'layout') -> Tuple[bool, str]:
    ...
    converter = FastPDFToWordConverter(pdf_path, output_path, mode)
...
def main():
    """CLI entry point"""
    if len(sys.argv) < 3:
        print("Usage: python pdf_to_word_fast.py <input_pdf> <output_docx> [layout|editable]")
        sys.exit(1)
